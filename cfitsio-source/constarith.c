...
int main(int argc, char *argv[])
{
	...
	if (argc > 5 || argc < 4 ) { 
		...
		printf("Added log, log10, sqrt and exp functions.\n");
		printf("\n");
		...
		printf("  constarith in.fit log out.fits - returns natural log of all pixels in the image.\n");
		printf("\n");
		return(0);
	}
	...
	if      (*argv[2] == 'a' || *argv[2] == 'A' || *argv[2] == '+' )
		...
		op = 5;
	else if ( !strcmp( argv[2], "log10") )
		op = 6;
	else if ( !strcmp( argv[2], "exp") )
		op = 8;
	else {
	...
	if (check && !fits_create_file(&outfptr, argv[argc-1], &status) )
	{
		...
		for (firstpix[2] = 1; firstpix[2] <= anaxes[2]; firstpix[2]++)
		{
			...
			for (firstpix[1] = 1; firstpix[1] <= anaxes[1]; firstpix[1]++)
			{
				...
				switch (op) {
					...
					case 6:
						for(ii=0; ii< npixels; ii++)
							apix[ii] = log10 ( apix[ii] );
						break;
					case 7:
						for(ii=0; ii< npixels; ii++)
							apix[ii] = sqrt ( apix[ii] );
						break;
					case 8:
						for(ii=0; ii< npixels; ii++)
							apix[ii] = exp ( apix[ii] );

...
int main(int argc, char *argv[])
{
	...
	if (argc != 5) { 
		printf("\n");
		printf("Usage: pruneimage image max/min min/max outimage \n");
		printf("\n");
		...
		printf("  with DATAMAX and DATAMIN keywords set accordingly\n");
		printf("\n");
		printf("Example: \n");
	...
	if (check && !fits_create_file(&outfptr, argv[4], &status) )
	{
		...
		/*	update_key creates a new key */
		/*	modify_key modifies existing key */
		if (fits_read_card(outfptr,"DATAMIN", card, &status))
		{
			status = 0;
			fits_update_key_flt (outfptr, "DATAMIN", minval, -10, "Keyword set by pruneimage", &status );
		} else {
			fits_modify_key_flt (outfptr, "DATAMIN", minval, -10, "&", &status );
		}

		if (fits_read_card(outfptr,"DATAMAX", card, &status))
		{
			status = 0;
			fits_update_key_flt (outfptr, "DATAMAX", maxval, -10, "Keyword set by pruneimage", &status );
		} else {

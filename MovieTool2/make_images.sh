...
TSIZE=128		#	Thumbnail Image size
...
read form_args
#echo $form_args > /tmp/jake030617b
...
#	Does binning, framing, resizing, ... effect this?  (I think so)
		...
		then
			...
			#
			MODEL=`$BIN/getbkgimg $CAMERA $DATEOBS $FILTER $POLAR`
...
#
		...
		then
			...
			$BIN/imagearith $tempfits \> /tmp/tempL0.$$.fits /tmp/temp55.$$.fits
			...
			$BIN/imagearith $tempfits \> /tmp/tempSL.$$.fits /tmp/temp60.$$.fits
			tempfits="/tmp/temp60.$$.fits"
		fi

		#
		#	PBM conversions are based on DATAMIN and DATAMAX
		#	keyword if they exist 
...
#		$BIN/pruneimage $tempfits 100 -100 /tmp/temp70.$$.fits
#		tempfits="/tmp/temp70.$$.fits"
	...
	then
		case $OUTTYPE in
			JPG)	echo "Converting FITS to JPG.<br>\n"
				echo "</TD>"
				$FITSTOPGM $tempfits | $PNMFLIP | $PNMSCALE $OUTSIZE $OUTSIZE | $TOJPG > $OUTDIR.$OUTSIZE/$outroot.$OUTSIZE.jpg
				if [ "$OUTSIZE" != $TSIZE ]
				then
					$FITSTOPGM $tempfits | $PNMFLIP | $PNMSCALE $TSIZE $TSIZE | $TOJPG > $OUTDIR.$TSIZE/$outroot.$TSIZE.jpg
				fi
				echo "<TD WIDTH=150>"
				echo "<img src='$WWWURL/MovieTool/$DATE.$$.$TSIZE/$outroot.$TSIZE.jpg'><BR>\n"
				echo "</TD>"
				;;
			PNG|MNG)	echo "Converting FITS to PNG.<br>\n"
				echo "</TD>"
				$FITSTOPGM $tempfits | $PNMFLIP | $PNMSCALE $OUTSIZE $OUTSIZE | $TOPNG > $OUTDIR.$OUTSIZE/$outroot.$OUTSIZE.png
				if [ "$OUTSIZE" != $TSIZE ]
				then
					$FITSTOPGM $tempfits | $PNMFLIP | $PNMSCALE $TSIZE $TSIZE | $TOPNG > $OUTDIR.$TSIZE/$outroot.$TSIZE.png
				fi
				echo "<TD WIDTH=150>"
				echo "<img src='$WWWURL/MovieTool/$DATE.$$.$TSIZE/$outroot.$TSIZE.png'><BR>\n"
				echo "</TD>"
				;;
			*)	echo "Converting FITS to GIF.<br>\n"
				echo "</TD>"
				$FITSTOPGM $tempfits | $PNMFLIP | $PNMSCALE $OUTSIZE $OUTSIZE | $TOGIF > $OUTDIR.$OUTSIZE/$outroot.$OUTSIZE.gif
				if [ "$OUTSIZE" != $TSIZE ]
				then
					$FITSTOPGM $tempfits | $PNMFLIP | $PNMSCALE $TSIZE $TSIZE | $TOGIF > $OUTDIR.$TSIZE/$outroot.$TSIZE.gif
				fi
				echo "<TD WIDTH=150>"
				echo "<img src='$WWWURL/MovieTool/$DATE.$$.$TSIZE/$outroot.$TSIZE.gif'><BR>\n"
				echo "</TD>"
				;;
		esac
	else
		echo "Not creating GIF.<br>\n"
		echo "</TD><TD></TD>"
	fi
	echo "</TR>"
done
